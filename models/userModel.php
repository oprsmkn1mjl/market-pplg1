...
function insert($data)
{
  ...
  if (mysqli_num_rows($cekUsername) > 0) {
    echo '<script>alert("Maaf username sudah terpakai");</script>';
    return false;
  }
...
function delete($id, $foto)
{
  ...
  $sqlData = "DELETE FROM user WHERE userid = '$id'";
...
function update($data)
{
  ...
  mysqli_query($koneksi, "UPDATE user SET username = '$username', fullname = '$fullname', address = '$address', level = '$level', foto = '$imgUser' WHERE userid = '$iduser'");
